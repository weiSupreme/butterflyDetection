...
############################################################################################################
        path = r'butterfly/annotations_txt/' + file_name_list[i].rstrip('.jpg')+'.txt'
        #print 'csv path: ', path
        ...
        for item in result:
            ...
            for line in read_txt:
                if csv_line_num == -1:
                    ...
                    continue
                line = line.split(' ')
                xminT = (int)(float(line[0])) if (int)(float(line[0])) > 1 else 1
                yminT = (int)(float(line[1])) if (int)(float(line[1])) > 1 else 1
                xmaxT = (int)(float(line[2])) if (int)(float(line[2])) < 500 else 500
                ymaxT = (int)(float(line[3])) if (int)(float(line[3])) < 500 else 500
...
#########################################################################################################          
        ...
        img.save('butterfly/detect_results/'+file_name_list[i])
...
def parse_args():
    ...
    if VGG16 == 1:
        ...
        parser.add_argument('--model_weights',
                        default='models/VGGNet/butterfly/butterfly_500x500/VGG_butterfly_butterfly_500x500_iter_5000.caffemodel')
    else:
        parser.add_argument('--model_def',
                            default='models/VGGNet/butterfly/butterfly_500x500/deploy.prototxt')
        parser.add_argument('--model_weights',
